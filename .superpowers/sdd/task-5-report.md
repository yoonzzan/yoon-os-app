...
- canonical key에는 JavaScript의 case/NFKC/whitespace 재정규화를 전혀 적용하지 않는다. backend SSOT을 신뢰하고 string·non-empty, Unicode code-point 기준 최대 80, 비허용 control 차단, exact metadata shape 및 duplicate/node-membership 불변식만 검사한다. `normalize_tag_value('ΐ') == 'ΐ'`의 post-casefold decomposed key, 80 astral code-point 허용과 81/control/membership mismatch 거부 회귀를 추가했다.
- tag raw display도 Python과 같은 Unicode code-point 기준 최대 256으로 검증한다. `'#' * 100 + '😀' * 80`의 180 code-point/260 UTF-16-unit producer output을 허용하고, 257 code-point는 거부하는 회귀를 추가했다.
- 공통 graph `safeText()`도 Python `len()`과 같은 Unicode code-point 기준 max length를 사용하며 type/control/multiline 규칙은 유지한다. 1024/1025 astral `source_locator`, 4000/4097 astral multiline `link.raw_text` 경계 회귀를 추가했다.
